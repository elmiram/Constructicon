...
from TestCorpus.search import pages
...
class Root(BaseStorageView):
    ...
    def get(self, request):
        if len(request.GET) < 1:
            ...
            return render_to_response('annotate_list.html', {'docs': doc_list, 'users': User.objects.exclude(username='admin').exclude(first_name='')}, context_instance=RequestContext(request))
        else:
            if 'user' in request.GET.keys():
                ...
                return render_to_response('annotate_list.html', {'docs': doc_list2, 'users': User.objects.exclude(username='admin').exclude(first_name='')}, context_instance=RequestContext(request))
            else:
